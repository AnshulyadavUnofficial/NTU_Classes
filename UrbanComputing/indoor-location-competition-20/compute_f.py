...
def split_ts_seq(ts_seq, sep_ts):
    """
    Split a time-series sequence into sub-sequences based on separator timestamps.

    Parameters
    ----------
    ts_seq : numpy.ndarray
        Time-series data with timestamps in the first column.
    sep_ts : numpy.ndarray
        Timestamps used as separators (split points).

    Returns
    -------
    ts_seqs : list of numpy.ndarray
        List of sub-sequences, each sliced from ts_seq.
    """
    # Extract timestamps from the first column and cast to float
    tss = ts_seq[:, 0].astype(float)

    # Ensure separator timestamps are unique and sorted
    unique_sep_ts = np.unique(sep_ts)

    ts_seqs = []
    start_index = 0

    # Iterate through each separator timestamp
    for i in range(unique_sep_ts.shape[0]):
        # Find the index where sep_ts[i] would be inserted
        # to maintain order in tss (right = include equal timestamps)
        end_index = np.searchsorted(tss, unique_sep_ts[i], side='right')

        # If no new data falls into this segment, skip
        if start_index == end_index:
            continue

        # Append the segment between [start_index, end_index)
...
def compute_step_positions(acce_datas, ahrs_datas, posi_datas):
    """
    Compute absolute step positions using sensor fusion of accelerometer, orientation, 
    and reference position data. This function implements a Pedestrian Dead Reckoning (PDR)
    system that estimates footstep positions and corrects drift using reference points.
    
    The processing pipeline:
    1. Detect steps from accelerometer data
    2. Calculate device orientation headings from AHRS data
    3. Estimate stride lengths from step characteristics
    4. Assign headings to each detected step
    5. Compute relative position displacements from strides and headings
    6. Correct trajectory using reference positions to minimize cumulative error
    
    Parameters:
    -----------
    acce_datas : numpy.ndarray
        Accelerometer data with format [timestamp, ax, ay, az]
    ahrs_datas : numpy.ndarray
        Orientation data (AHRS) with format [timestamp, q1, q2, q3, (q0)]
    posi_datas : numpy.ndarray
        Reference position data with format [timestamp, x, y]
        
    Returns:
    --------
    step_positions : numpy.ndarray
        Corrected absolute positions for each step with format [timestamp, x, y]
    """
    # Step detection from accelerometer data
    step_timestamps, step_indexs, step_acce_max_mins = compute_steps(acce_datas)
    
    # Calculate device orientation (heading) from AHRS data
    headings = compute_headings(ahrs_datas)
    
    # Estimate stride lengths using step acceleration characteristics
    stride_lengths = compute_stride_length(step_acce_max_mins)
    
    # Assign orientation headings to each detected step
    step_headings = compute_step_heading(step_timestamps, headings)
    
    # Compute relative position changes from stride lengths and headings
    rel_positions = compute_rel_positions(stride_lengths, step_headings)
    
    # Correct trajectory using reference positions to minimize drift
